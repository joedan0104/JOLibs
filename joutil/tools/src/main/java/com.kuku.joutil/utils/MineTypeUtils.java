...
public class MineTypeUtils {
    ...
    public static String getMimeType(File file) {
        String fileType = StringUtils.stringNoNil(FileTypeUtils.getFileType(file));
        String mimeType = MIME_TYPE_MAP.get(fileType);
        if (TextUtils.isEmpty(mimeType)) {
            // 如果没有找到对应的MineType，获取默认的值
            mimeType = MIME_TYPE_MAP.get("");
        }
        return mimeType;
    }
}
