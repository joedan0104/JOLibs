...
public final class ThreadPoolUtils {
    ...
    public static ThreadPoolUtils EXCUTOR = getInstance();

    private static ThreadPoolUtils getInstance() {
        if(null == sInstance) {
            synchronized (ThreadPoolUtils.class) {
                if(null == sInstance) {
                    sInstance = new ThreadPoolUtils(CachedThread, 5);
                }
            }
        }
        return sInstance;
